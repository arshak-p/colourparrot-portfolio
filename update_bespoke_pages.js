...
function updateFile(filename, componentImport) {
    ...
    // Inject the process component import if needed
    if (componentImport && !content.includes('ServiceProcess')) {
        content = content.replace(
            `import { Link } from 'react-router-dom';`,
            `import { Link } from 'react-router-dom';\n${componentImport}`
        );

        if (filename === 'WebDesignPage.jsx') {
            content = content.replace(
                `{/* Feature Grid */}`,
                `{/* Step-by-step Process Section */}\n      <ServiceProcess accent={service.accent} steps={webDesignSteps} />\n\n      {/* Feature Grid */}`
            );
        } else if (filename === 'DigitalMarketingPage.jsx') {
            content = content.replace(
                `{/* Feature Grid */}`,
...
updateFile('WebDesignPage.jsx', `import { ServiceProcess, webDesignSteps } from '../ServiceProcess';`);
updateFile('DigitalMarketingPage.jsx', `import { ServiceProcess, digitalMarketingSteps } from '../ServiceProcess';`);
updateFile('VideoProductionPage.jsx', '');
updateFile('ContentCreationPage.jsx', '');
updateFile('CreativeAdvertisingPage.jsx', '');

console.log('Bespoke pages updated!');
